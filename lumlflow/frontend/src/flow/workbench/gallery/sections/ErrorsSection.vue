<template>
  <div class="flex flex-col gap-8 max-w-5xl">
    <GallerySpecimen
      ...
      caption="Agent-authored → demoted, not suppressed: the chip goes failed and the traceback fills logs — no toast, no red wash. Notebook density may show the summary quietly under the code, because code is the subject. History folds to “v3→v4 · 1 failed attempt”."
    >
      <div class="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <div class="flex flex-col gap-1.5 min-w-0">
          <p class="text-[11px] uppercase tracking-wide text-muted-color">canvas density</p>
          <CellCard :cell="agentFailedCell" density="canvas" v-on="cardEvents(agentFailedCell)" />
        </div>
        <div class="flex flex-col gap-1.5 min-w-0">
          <p class="text-[11px] uppercase tracking-wide text-muted-color">notebook density</p>
    ...
    <GallerySpecimen
      ...
      caption="User-authored → loud: inline error summary on the card, the full traceback in logs, and a Fix-this handoff preloaded with the error — the difference between showing an error and doing something about it."
    >
      <div class="max-w-2xl">
    ...
    <GallerySpecimen
      ...
      caption="A broken declaration is accepted but flagged, never rejected — agents iterate through broken intermediate states. The suggestion is one click away."
    >
      <div class="max-w-2xl">
    ...
    <GallerySpecimen
      title="Conflict — edit landed on a moved head"
      caption="Optimistic locking rejected the edit into a menu: fork-my-edit is promoted because it loses nothing; overwrite is secondary."
    >
      <div class="max-w-2xl">
    ...
    <GallerySpecimen
      ...
      caption="A persistent inline banner — a state, not a toast — anchored under the last cell the agent touched. It says what is outstanding, never why the session ended, and offers the handoff payload."
    >
      <AgentEndedBanner
    ...
    <GallerySpecimen
      title="Kernel death"
      caption="Names the cell that was materializing when the kernel died, offers restart, and states what is true: nothing recorded is lost, and the queue is drained — not silently retried."
    >
      <KernelDeathBanner
        slug="train_model"
        cause="out of memory"
        @restart-kernel="note('would restart the kernel under this branch\'s lock')"
      />
    </GallerySpecimen>
  </div>
</template>
