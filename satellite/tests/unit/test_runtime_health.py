...
class TestRuntimeHealth:
    ...
    def test_counts_and_error_rate(self) -> None:
    ...
    def test_failed_inference_counts_only_incomplete_inferences(self) -> None:
        events = [
    ...
    def test_healthy_window_has_no_signals(self) -> None:
        result = _compute([_event() for _ in range(100)], latency_p95_threshold_ms=1000.0)

        assert result.severity == Severity.NORMAL
        assert result.signals == []

    def test_error_rate_warning_threshold(self) -> None:
        events = [_event() for _ in range(97)] + [
            _event(status="error", code=400) for _ in range(3)
        ]
        result = _compute(events)
    ...
    def test_error_rate_critical_threshold(self) -> None:
        events = [_event() for _ in range(90)] + [
            _event(status="error", code=500) for _ in range(10)
        ]
        result = _compute(events)
    ...
    def test_latency_p95_over_threshold_warns(self) -> None:
    ...
    def test_applies_requires_events(self) -> None:
