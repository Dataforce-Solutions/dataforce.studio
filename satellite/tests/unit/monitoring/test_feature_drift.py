...
class TestFeatureDrift:
    async def test_feature_drift_ranked_by_psi_with_status(self) -> None:
        dep = uuid.uuid4()
    ...
    async def test_feature_drift_selected_feature_returns_distribution_and_psi_over_time(
        self,
    ) -> None:
        dep = uuid.uuid4()
        ...
        assert [b.label for b in selected.distribution.bins] == [
            "[0,10k)",
            "[10k,20k)",
            "[20k,inf)",
        ]
    ...
    async def test_feature_drift_no_selected_feature_when_filter_unset(self) -> None:
        dep = uuid.uuid4()
        result = await _service(_drift_store(dep)).feature_drift(dep, _dims())

        assert result.selected is None

    async def test_feature_drift_unknown_selected_feature_is_none(self) -> None:
        dep = uuid.uuid4()
    ...
    async def test_multivariate_panel_returns_shift_variance_and_projection(self) -> None:
        dep = uuid.uuid4()
        result = await _service(_drift_store(dep)).feature_drift(dep, _dims())
    ...
    async def test_multivariate_panel_empty_when_only_univariate_computed(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
    ...
    async def test_feature_drift_empty_shape_when_not_computed(self) -> None:
        dep = uuid.uuid4()
    ...
    async def test_feature_drift_window_dimension_changes_the_query(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
    ...
    async def test_feature_drift_scoped_to_deployment(self) -> None:
    ...
    async def test_feature_drift_severity_filter_narrows_alerts_not_features(self) -> None:
        dep = uuid.uuid4()
    ...
    async def test_feature_drift_placeholder_profile_degrades_gracefully(self) -> None:
        dep = uuid.uuid4()
        store = _drift_store(dep)
    ...
    async def test_feature_drift_store_unavailable_yields_unavailable_state(self) -> None:
        dep = uuid.uuid4()
        store = _drift_store(dep)
    ...
    async def test_reference_profile_selected_numeric_feature(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
    ...
    async def test_reference_profile_selected_categorical_feature(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
        store.add_profile(_reference_profile(dep))
    ...
    async def test_reference_profile_lists_features_without_selection(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
        store.add_profile(_reference_profile(dep))
    ...
    async def test_reference_profile_empty_shape_when_absent(self) -> None:
        dep = uuid.uuid4()
    ...
    async def test_reference_profile_placeholder_degrades_gracefully(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
    ...
    async def test_reference_profile_carries_the_whole_document(self) -> None:
    ...
    async def test_reference_profile_store_unavailable_yields_unavailable_state(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
        store.add_profile(_reference_profile(dep))
        store.unavailable = True
    ...
    async def test_psi_over_time_is_assembled_from_past_windows(self) -> None:
    ...
    async def test_psi_over_time_needs_more_than_one_window(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
    ...
    async def test_multivariate_panel_shows_the_univariate_psi_ranking(self) -> None:
