...
class TestSessionScope:
    async def test_sessions_derive_isolated_deployments(self) -> None:
        ...
        async with client_for(app) as client:
            resp_a = await client.get(
                "/monitoring/api/session", headers=_cookie(session_a.session_id)
            )
            resp_b = await client.get(
                "/monitoring/api/session", headers=_cookie(session_b.session_id)
            )
    ...
    async def test_client_supplied_deployment_id_is_ignored(self) -> None:
        store = MonitoringSessionStore()
        session_a = store.create(uuid.uuid4(), MONITORING_READ_SCOPE)
    ...
    async def test_missing_session_cookie_is_unauthorized(self) -> None:
    ...
    async def test_unknown_session_id_is_unauthorized(self) -> None:
        app = build_app(_INACTIVE)
    ...
    async def test_expired_session_is_unauthorized(self) -> None:
        now = [1000.0]
