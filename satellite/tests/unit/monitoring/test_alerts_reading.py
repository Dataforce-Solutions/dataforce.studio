...
class TestAlertsReading:
    def test_key_splits_into_group_and_feature(self) -> None:
    ...
    def test_dotted_feature_names_keep_their_dots(self) -> None:
    ...
    def test_values_are_phrased_in_their_own_units(self) -> None:
    ...
    def test_history_is_read_from_the_group_own_layout(self) -> None:
        quality = {"features": {"region": {"unseen_category_rate": 0.03, "missing_rate": 0.0}}}
        assert (
            history_value(parse_alert_key("data_quality:region.unseen_category"), quality) == 0.03
        )
        ...
        assert (
            history_value(
                parse_alert_key("multivariate:mahalanobis_outliers"), {"outlier_rate": 0.15}
            )
            == 0.15
        )

    async def test_alert_carries_its_wording_duration_and_history(self) -> None:
        dep = uuid.uuid4()
    ...
    async def test_one_window_is_not_a_history(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
        store.add_alert(_alert(dep, "feature_drift:income", 0.42, 0.25))
