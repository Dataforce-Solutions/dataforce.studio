...
class TestWorkerHealth:
    async def test_a_healthy_tick_records_what_it_did(self) -> None:
        health = WorkerHealth()
    ...
    async def test_a_failing_metric_is_remembered_not_just_logged(self) -> None:
        health = WorkerHealth()
    ...
    async def test_a_metric_that_recovers_clears_its_failure(self) -> None:
        health = WorkerHealth()
    ...
    async def test_the_dashboard_can_read_the_workers_state(self) -> None:
        health = WorkerHealth()
        await _worker(health, _ExplodingMetric()).tick(now=NOW)
        dep = uuid.uuid4()
    ...
    async def test_without_a_worker_the_section_is_unavailable(self) -> None:
    ...
    async def test_dimensions_are_not_needed_to_ask_about_the_worker(self) -> None:
    ...
    async def test_failure_history_survives_the_process(self) -> None:
    ...
    async def test_only_transitions_are_written_not_every_tick(self) -> None:
    ...
    async def test_recovery_closes_the_incident(self) -> None:
