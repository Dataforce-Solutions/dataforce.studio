...
class TestMachineAPI:
    async def test_a_key_reads_any_section_of_a_hosted_deployment(self, app) -> None:  # noqa: ANN001
    ...
    async def test_without_a_credential_the_door_stays_shut(self, app) -> None:  # noqa: ANN001
        async with _client(app) as client:
            resp = await client.get(_url())

        assert resp.status_code == 403

    async def test_a_rejected_key_is_unauthenticated(self, app) -> None:  # noqa: ANN001
        async with _client(app) as client:
            resp = await client.get(_url(), headers=_bearer("dfs_wrong"))

        assert resp.status_code == 401

    async def test_an_unverifiable_key_fails_closed(self, app) -> None:  # noqa: ANN001
    ...
    async def test_a_deployment_this_satellite_does_not_host_is_not_found(self, app) -> None:  # noqa: ANN001
        async with _client(app) as client:
    ...
    async def test_an_unmatched_route_has_a_machine_readable_code(self, app) -> None:  # noqa: ANN001
        async with _client(app) as client:
    ...
    async def test_a_missing_dashboard_static_file_keeps_its_plain_404(self, app) -> None:  # noqa: ANN001
        async with _client(app) as client:
    ...
    async def test_a_missing_trace_keeps_its_plain_404(self, app) -> None:  # noqa: ANN001
        async with _client(app) as client:
    ...
    async def test_a_dashboard_cookie_does_not_open_the_machine_surface(self, app) -> None:  # noqa: ANN001
    ...
    async def test_the_machine_surface_is_read_only(self, app) -> None:  # noqa: ANN001
    ...
    async def test_the_listing_says_what_is_monitored_here(self, app, mock_model_server) -> None:  # noqa: ANN001
