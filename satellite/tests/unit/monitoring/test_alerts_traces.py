...
class TestAlertsTraces:
    async def test_alerts_grouped_by_metric_group_in_canonical_order(self) -> None:
        dep = uuid.uuid4()
    ...
    async def test_alerts_severity_filter_narrows_to_critical(self) -> None:
        dep = uuid.uuid4()
    ...
    async def test_alerts_excludes_resolved_alerts(self) -> None:
        dep = uuid.uuid4()
        ...
        runtime_metrics = [
            a.metric for g in result.groups if g.group == "runtime" for a in g.alerts
        ]
        assert runtime_metrics == ["error_rate"]  # only the open alert, resolved one excluded

    async def test_alerts_empty_when_none_open_is_ok_not_missing(self) -> None:
        dep = uuid.uuid4()
        result = await _service(InMemoryMonitoringStore()).alerts(dep, _dims())

        assert (
            result.state is SectionState.OK
        )  # no open alerts is a healthy answer, not "not computed"
        assert result.groups == []

    async def test_alerts_scoped_to_deployment(self) -> None:
    ...
    async def test_alerts_store_unavailable_yields_unavailable_state(self) -> None:
        dep = uuid.uuid4()
        store = _alerts_store(dep)
    ...
    async def test_traces_returns_recent_calls_most_recent_first(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
    ...
    async def test_traces_summarize_features_and_prediction(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
        store.add_event(
            _trace_event(
    ...
    async def test_traces_paginate_with_limit_and_offset(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
    ...
    async def test_traces_offset_beyond_total_is_ok_with_no_rows(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
    ...
    async def test_traces_respects_window(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
    ...
    async def test_traces_empty_shape_when_no_events(self) -> None:
        dep = uuid.uuid4()
    ...
    async def test_trace_detail_returns_full_payloads_not_summaries(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
        store.add_event(
            _trace_event(
    ...
    async def test_trace_detail_keeps_non_json_payload_as_text(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
    ...
    async def test_trace_detail_missing_event_is_empty(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
        store.add_event(_trace_event(dep, 100))
    ...
    async def test_trace_detail_returns_every_span_of_the_trace(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
    ...
    async def test_trace_detail_attaches_payloads_to_the_root_span_only(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
        store.add_event(
            _trace_event(
    ...
    async def test_trace_detail_synthesizes_one_span_when_none_were_collected(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
    ...
    async def test_traces_only_show_the_sessions_deployment(self) -> None:
    ...
    async def test_trace_detail_cannot_open_another_deployments_call(self) -> None:
        mine, other = uuid.uuid4(), uuid.uuid4()
        store = InMemoryMonitoringStore()
    ...
    async def test_trace_detail_scoped_to_window(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
        store.add_event(_trace_event(dep, 25 * 3600))  # outside 24h, inside 7d
    ...
    async def test_traces_scoped_to_deployment(self) -> None:
        dep_a, dep_b = uuid.uuid4(), uuid.uuid4()
        store = InMemoryMonitoringStore()
        for _ in range(3):
    ...
    async def test_traces_store_unavailable_yields_unavailable_state(self) -> None:
        dep = uuid.uuid4()
        store = InMemoryMonitoringStore()
        store.add_event(_trace_event(dep, 100))
        store.unavailable = True
