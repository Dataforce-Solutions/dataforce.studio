...
class TestMonitoringFeatureDrift:
    def test_applies_requires_events_and_feature_summaries(self) -> None:
    ...
    def test_stable_numerical_stays_normal(self) -> None:
    ...
    def test_shifted_numerical_raises_critical(self) -> None:
    ...
    def test_moderate_numerical_shift_warns(self) -> None:
    ...
    def test_stable_categorical_stays_normal(self) -> None:
        values = (["a"] * 50) + (["b"] * 50)
    ...
    def test_shifted_categorical_raises_critical(self) -> None:
    ...
    def test_unseen_categories_contribute_to_psi(self) -> None:
        profile = _profile(categorical=CAT_REF)
        known = _compute(_events("region", (["a"] * 50) + (["b"] * 50)), profile)
        with_unseen = _compute(
            _events("region", (["a"] * 45) + (["b"] * 45) + (["z"] * 10)), profile
        )
    ...
    def test_feature_without_reference_distribution_is_skipped(self) -> None:
    ...
    def test_feature_with_no_valid_live_values_is_skipped(self) -> None:
        profile = _profile(numerical=NUM_REF)
        events = [_event({"age": None}) for _ in range(5)] + [
            _event({"age": "x"}) for _ in range(5)
        ]
    ...
    async def test_worker_materializes_feature_drift_and_opens_alert(self) -> None:
    ...
    async def test_worker_feature_drift_alert_resolves_when_data_recovers(self) -> None:
        store = InMemoryMonitoringStore()
        profile = _profile(numerical=NUM_REF)
    ...
    async def test_worker_skips_feature_drift_without_profile(self) -> None:
        store = InMemoryMonitoringStore()
    ...
    async def test_worker_runs_applicable_subset_with_feature_summaries(self) -> None:
    ...
    def test_feature_drift_aggregates_batched_observations(self) -> None:
        metric = FeatureDriftMetric()
    ...
    def test_numerical_result_carries_the_two_distributions_psi_compares(self) -> None:
    ...
    def test_categorical_distribution_includes_categories_only_seen_live(self) -> None:
