...
class TestArtifactEndpoint:
    def test_a_container_gets_a_url_signed_for_this_request(self) -> None:
    ...
    def test_without_a_token_nothing_is_handed_out(self) -> None:
        assert _client().get(_url()).status_code == 403

    def test_a_token_for_another_deployment_is_refused(self) -> None:
    ...
    def test_a_deployment_this_satellite_does_not_host_is_a_404(self) -> None:
    ...
    def test_a_platform_failure_is_reported_as_a_gateway_error(self) -> None:
        async def resolve(deployment_id: uuid.UUID) -> ArtifactDownload:
    ...
    def test_tokens_survive_a_restart_of_either_side(self) -> None:
    ...
    def test_presigned_expiry_is_read_when_it_is_there(
        self, url: str, expected: datetime | None
    ) -> None:
        assert presigned_expiry(url) == expected

    @respx.mock
    async def test_the_resolver_signs_a_url_for_a_deployment_this_satellite_hosts(self) -> None:
    ...
    @respx.mock
    async def test_the_resolver_turns_an_unknown_deployment_into_a_404(self) -> None:
