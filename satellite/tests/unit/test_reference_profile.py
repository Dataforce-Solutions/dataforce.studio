...
class TestReferenceProfile:
    def test_usable_reference_profile_returns_ready_profile(self) -> None:
        assert usable_reference_profile(READY_PROFILE) == READY_PROFILE

    def test_usable_reference_profile_treats_placeholder_as_no_profile(self) -> None:
        assert usable_reference_profile(PLACEHOLDER_PROFILE) is None

    def test_usable_reference_profile_treats_missing_status_as_no_profile(self) -> None:
        assert usable_reference_profile({"task_type": "regression"}) is None

    @pytest.mark.parametrize("profile", [None, {}])
    def test_usable_reference_profile_treats_empty_as_no_profile(
        self, profile: dict | None
    ) -> None:
        assert usable_reference_profile(profile) is None

    async def test_deploy_loads_reference_profile(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def test_deploy_without_profile_marks_no_profile(
        self, monkeypatch: pytest.MonkeyPatch, profile: dict | None
    ) -> None:
    ...
    async def test_client_returns_profile_on_200(self) -> None:
    ...
    async def test_client_returns_none_when_endpoint_missing(self) -> None:
        def handler(request: httpx.Request) -> httpx.Response:
    ...
    def test_usable_reference_profile_accepts_summaries_without_status(self) -> None:
        # Baseline has no profile_status; real feature_summaries make it usable.
