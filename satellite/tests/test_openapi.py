...
def test_machine_operations_have_facets_security_and_descriptions(app: FastAPI) -> None:
    schema = app.openapi()
    expected_facets = {
...
async def test_health_requires_bearer_but_inference_access_does_not(
    ...
    health_with_key = await client.get("/healthz", headers={"Authorization": f"Bearer {GOOD_KEY}"})
